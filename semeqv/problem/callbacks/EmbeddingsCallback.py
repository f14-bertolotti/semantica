...
class EmbeddingsCallback(LoggingCallback):
    ...
    def __init__(self, trainer, path="", step_log_path="", epoch_log_path="", steps_to_checkpoint=0): 
...
@click.pass_obj
def traincallback(trainer, path, step_log_path, epoch_log_path, stc):
    trainer.set_traincallback(EmbeddingsCallback(trainer, path=path, step_log_path=step_log_path, epoch_log_path=epoch_log_path, steps_to_checkpoint=stc))
...
@click.option("--stc" , "stc" , type=int, default=0)
@click.pass_obj
def validcallback(trainer, path, step_log_path, epoch_log_path,  stc):
    trainer.set_validcallback(EmbeddingsCallback(trainer, path=path, step_log_path=step_log_path, epoch_log_path=epoch_log_path,  steps_to_checkpoint=stc))
...
@click.option("--stc" , "stc" , type=int, default=0)
@click.pass_obj
def testcallback(trainer, path, step_log_path, epoch_log_path,  stc):
    trainer.set_testcallback(EmbeddingsCallback(trainer, path=path, step_log_path=step_log_path, epoch_log_path=epoch_log_path,  steps_to_checkpoint=stc))
